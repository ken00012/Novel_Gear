...
export default function StatusAttributeTab() {
    ...
    const handleCreate = async () => {
        if (!newItem.key || !newItem.name) return;
        try {
    ...
    const handleUpdate = async (id: number) => {
        if (!editItem.key || !editItem.name) return;
        try {
            ...
            {isCreating && (
                <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100 mb-6 flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-600 mb-1">内部キー (アルファベット推奨, 例: hp, str)</label>
                            <input
                                value={newItem.key || ''}
                    ...
                    <div className="flex justify-end gap-2 mt-2">
                        ...
                        <button onClick={handleCreate} disabled={!newItem.key || !newItem.name} className="px-4 py-1.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 rounded transition">追加する</button>
                    </div>
                </div>
            )}
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200 text-xs text-gray-500 uppercase">
                            ...
                            <th className="p-3 font-medium">内部キー</th>
                    ...
                    <tbody className="text-sm">
                        {attributes.map(attr => (
                            <tr key={attr.id} className="border-b last:border-b-0 hover:bg-gray-50 transition group">
                                <td className="p-3 text-gray-300 group-hover:text-gray-500 cursor-grab">
                                ...
                                {editingId === attr.id ? (
                                    <>
                                        ...
                                        <td className="p-3">
                                            <input value={editItem.key || ''} onChange={e => setEditItem({ ...editItem, key: e.target.value })} className="w-full p-1 border rounded text-sm font-mono" />
                                        </td>
                                        <td className="p-3">
                                    ...
                                    <>
                                        ...
                                        <td className="p-3 font-mono text-gray-500">{attr.key}</td>

...
import { api, type CharacterProfileAttribute, type Tag } from '../../api';
...
export default function ProfileAttributeTab() {
    ...
    const handleCreate = async () => {
        ...
        try {
            await api.post('/profile_attributes/', newItem);
        ...
        <div className="bg-white rounded-lg shadow min-h-[500px] flex flex-col">
            ...
            <div className="flex-1 p-6 overflow-y-auto">
                ...
                <div className="space-y-3">
                    {attributes.map(attr => (
                        <div key={attr.id} className="border border-gray-200 rounded-lg bg-white overflow-hidden shadow-sm hover:shadow transition">
                            <div className="p-4 flex items-center gap-4">
                                {editingId === attr.id ? (
                                    ...
                                    <>
                                        <div className="w-48 bg-gray-50 border border-gray-200 rounded px-3 py-1 font-mono text-xs text-gray-500 truncate" title={attr.key}>
                                            {attr.key}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            {attr.type === 'tag' && (
                                                <button
                                                    onClick={() => setManagingTagsFor(managingTagsFor?.id === attr.id ? null : attr)}
                                            ...
                                            <button onClick={() => { setEditingId(attr.id); setEditItem(attr); }} className="text-gray-400 hover:text-indigo-600 p-2 transition">
                                                <Edit2 size={16} />
                                            </button>
                                            <button onClick={() => handleDelete(attr.id)} className="text-gray-400 hover:text-red-500 p-2 transition">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    </>
